...
`sqlite-objstore` is a SQLite extension that exposes an `objstore(id, data)` virtual table with pluggable backends (native filesystem, OPFS, VFS, SQLite). The architecture, invariants, and roadmap now live in [`docs/architecture.md`](docs/architecture.md); transactional nuances, tuning advice, and platform notes live in the docs listed below.
...
- [`docs/transactions.md`](docs/transactions.md) – commit hooks, snapshot behaviour, and savepoint limitations.
...
See [`docs/getting-started.md`](docs/getting-started.md) for a narrated walkthrough that covers building, installing, and embedding the extension.
...
`sqlite-objstore` is released under the [MIT License](LICENSE).
