...
Gemma via LiteRT-LM or pinned LFM2 Q4_K_M GGUFs via llama.cpp. There is no
embedding-only extractive fallback when no deep backend is available.
Note: Consolidation candidates are limited to LONG_TERM/ASSOCIATION memories
with non-null blob_id so summarization always has source text.
...
using ImageBind embeddings and a knob-derived baseline salience; this
