...
embeddings onto a discrete emotion space C = {anger, fear, joy, love, sadness,
surprise}. Categories inspired by @russell1980circumplex circumplex model are
projected onto the valence-arousal plane with the following coordinates:
...
The emotion intensity combines peak probability with distributional confidence
through a Sensitivity-dependent power transform of their product. It is high
only when a category dominates and the distribution has high confidence.
...
The emotional state acts as a modulator for memory encoding/consolidation,
following @mcgaugh2004amygdala. High emotion intensity and arousal increase the
likelihood of threshold crossings; valence is recorded as a separate metric but
does not enter this threshold delta directly.
...
Where `retrieval_pressure` is the normalized retrieval queue-depth input, and
`δ_reward_t` is a reward prediction error derived from downstream outcome
signals (see @sec-structural-metrics). The production operation context does not
currently populate queue depth, so this term is zero in the shipped full
pipeline; tests and experiment hooks can supply it. The other phasic terms are
live. These modulators drive internal gates without introducing new user-facing
parameters:
