...
9.  **Post-Write Updates and Retrieval:**
    ...
    *   Cache `q_retrieval ← e_rep` (or `μ_acc` fallback) before any accumulator reset.
...
10. **Run Interrupt Gate:** Check for streaming interrupt using retrieved candidates (already filtered by write‑exclusion and WM overlap during retrieval).
