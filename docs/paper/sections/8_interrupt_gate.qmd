...
When the interrupt gate allows a retrieval outside a flush/spike event, we mark a **pending abort** for the current accumulator to avoid persisting partial thoughts. If the next signal continues the same thought (low novelty relative to the current centroid), we **resume** the accumulator; otherwise we **drop** the partial unit. This prevents “half‑utterances” from being stored when a memory whisper redirects attention, while still allowing the speaker to ignore the interrupt and continue seamlessly. Because the resume test reuses the knob‑derived novelty threshold, no new fixed constants are introduced.
