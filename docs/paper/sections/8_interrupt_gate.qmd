...
The interrupt gate controls when retrieved memories enter active context during streaming generation. The gate balances novelty value against disruption cost. The interrupt gate operates on memory-level context, using centroids (μ_acc) rather than individual signal embeddings for novelty and relevance computation. The main interrupt thresholds and gains in this section are derived from the three knobs (F, S, T). The runtime also contains a small number of operational guard constants and environment-controlled diagnostics outside this core gate.
