...
Drift measures the instantaneous step between consecutive signals rather than a smoothed centroid lag:
...
This uses the current signal velocity in embedding space, ensuring drift responds immediately to context shifts. A threshold defines an informational drift-boundary signal:
...
We model real-time expectation using an exponential moving average (EMA) of the signal stream. The predictor state `x_pred_ema` represents the current expectation; low Stability (T) adapts quickly, high Stability adapts slowly.
