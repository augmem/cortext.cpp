...
Retrieval combines vector similarity with graph expansion. The query vector is the **current accumulator centroid** (μ_acc), which aggregates the embeddings of the signals observed so far in the unflushed memory. This makes retrieval responsive to the evolving thought unit rather than any single micro‑signal.
