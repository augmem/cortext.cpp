...
**Summarization + labeling engine:** Consolidation is the only stage that invokes a generative model. Summaries and labels are produced by **gemma-3n-e2b** running through `third_party/litert_lm`. All other operations remain embedding-only. **There is no extractive fallback**: if Gemma is unavailable or summarization fails, consolidation raises an error rather than silently degrading.
