...
where signal_gap_s = now_s() − to_s(last_signal_ts) is computed at the start of signal processing (before last_signal_ts is updated). Gap timing influences boundary_score via gap_score and sets the adaptive cadence used by max_mem_time, avoiding a fixed flush timer.
