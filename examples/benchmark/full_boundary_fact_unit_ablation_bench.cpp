...
constexpr std::array<UnitInfo, kUnitCount> kUnits = {
  UnitInfo{ "boundary_pressure" },
  UnitInfo{ "boundary_surprisal" },
  UnitInfo{ "boundary_natural" },
  UnitInfo{ "fact_layer" },
  UnitInfo{ "fact_history" },
  UnitInfo{ "fact_stale_penalty" },
  UnitInfo{ "fact_provenance" },
};
...
bool
UnitEnabled (std::uint32_t mask, int bit)
{
...
public:
  explicit ScopedMaskEnv (std::uint32_t mask)
  {
    Configure ("CORTEXT_BOUNDARY_DISABLE_PRESSURE",
               UnitEnabled (mask, kBoundaryPressure));
    Configure ("CORTEXT_BOUNDARY_DISABLE_SURPRISAL",
               UnitEnabled (mask, kBoundarySurprisal));
    Configure ("CORTEXT_BOUNDARY_DISABLE_NATURAL",
               UnitEnabled (mask, kBoundaryNatural));
  }
...
RunBoundaryPressureScenario (std::uint32_t mask)
{
  ...
  cortext::OperationContext ctx (MakeSignal (MakeVec (1.0f, 0.0f), 1200), pctx, cfg);
...
RunFactLayerScenario (std::uint32_t mask)
{
  ...
  ov.fact_layer_enabled = UnitEnabled (mask, kFactLayer);
...
RunFactHistoryScenario (std::uint32_t mask)
{
  ...
  ov.history_enabled = UnitEnabled (mask, kFactHistory);
  const auto order = RunFactRetrieval (store, query, 8000ULL,
...
RunFactStalePenaltyScenario (std::uint32_t mask)
{
  ...
  ov.fact_layer_enabled = true;
  ov.history_enabled = true;
  ov.stale_penalty_strength = UnitEnabled (mask, kFactStalePenalty)
...
RunFactProvenanceScenario (std::uint32_t mask)
{
  ...
  ov.fact_layer_enabled = true;
  ov.history_enabled = true;
  ov.provenance_mode = UnitEnabled (mask, kFactProvenance)
...
RunSuite (std::uint32_t mask)
{
  ComboResult out;
  out.mask = mask;
  out.scores[kBoundaryPressure] = RunBoundaryPressureScenario (mask);
...
main ()
{
  const std::uint32_t full_mask = (1u << kUnitCount) - 1u;
  const std::uint32_t combo_count = (1u << kUnitCount);
  ...
  for (const auto &r : results)
    {
      ...
      if (r.mask == full_mask)
        {
          full_score = r.total;
          full_scores = r.scores;
        }
    }

  std::cout << std::fixed << std::setprecision (6);
  std::cout << "boundary_fact_unit_count=" << kUnitCount << "\n";
  std::cout << "boundary_fact_unit_combo_count=" << combo_count << "\n";
  std::cout << "boundary_fact_unit_full_mask_score=" << full_score << "\n";
  std::cout << "boundary_fact_unit_best_score=" << best_score << "\n";
  std::cout << "boundary_fact_unit_full_mask_is_best="
            << (full_score == best_score ? 1 : 0) << "\n";
  for (int bit = 0; bit < kUnitCount; ++bit)
    {
      std::cout << "boundary_fact_unit_full_mask_component_"
                << kUnits[static_cast<std::size_t> (bit)].slug << "="
                << full_scores[static_cast<std::size_t> (bit)] << "\n";
    }
  std::cout << "\n| unit | mean_marginal | max_score_without | essential_for_best |\n";
  std::cout << "|---|---:|---:|---:|\n";
  for (int bit = 0; bit < kUnitCount; ++bit)
    {
      double sum = 0.0;
  ...
  for (std::size_t i = 0; i < std::min<std::size_t> (8, sorted.size ()); ++i)
    {
      std::cout << "| " << (i + 1) << " | " << sorted[i]->total << " | "
                << BitCount (sorted[i]->mask) << " | "
                << MaskSummary (sorted[i]->mask) << " | "
