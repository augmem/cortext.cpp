...
main ()
{
  ...
  const std::vector<Fixture> fixtures = {
    ...
    { "neighbor_housing",
      ...
      { "Sarah", "housing application", "Logan Square", "pay stubs", "Tuesday" },
      { "the user", "the assistant", "conversation", "excerpt", "discussed",
        "said", "asked", "user:", "assistant:", "final summary",
        "output summary", "json response",
        "underlying facts and topics could include" } },
  ...
  bool ok = true;
  for (const auto &[name, structured] : variants)
    {
      ScopedEnvVar prompt_style ("CORTEXT_LFM2_SUMMARY_PROMPT_STYLE",
                                 "transcript_fewshot");
      ScopedEnvVar prompt_structured ("CORTEXT_LFM2_SUMMARY_STRUCTURED",
                                      structured);
      VariantStats stats;
      stats.name = name;
      ...
      for (const auto &fixture : fixtures)
        {
          ...
          std::cout << "fixture_" << fixture.name << "_" << name
                    << "_summary=" << summary << "\n";
          std::cout << "fixture_" << fixture.name << "_" << name
                    << "_desired_hits=" << seen_desired.size () << "\n";
          std::cout << "fixture_" << fixture.name << "_" << name
                    << "_banned_hits=" << seen_banned.size () << "\n";
        }
      ...
      std::cout << "summary_structured_ablation_" << name << "_desired_hits="
                << stats.desired_hits << "\n";
      std::cout << "summary_structured_ablation_" << name << "_banned_hits="
                << stats.banned_hits << "\n";
      std::cout << "summary_structured_ablation_" << name
                << "_summaries_emitted=" << stats.summaries_emitted << "\n";
      std::cout << "summary_structured_ablation_" << name << "_latency_ms="
                << stats.latency_ms << "\n";
      std::cout << "summary_structured_ablation_" << name << "_score="
                << stats.score << "\n";
