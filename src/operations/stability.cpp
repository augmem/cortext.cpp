          ...
          try
            {
              ...
              auto rows = tx.Execute (
                  ...
                  "  SELECT m.created_at, m.start_ts "
                  "  FROM memories m INDEXED BY idx_memories_ltm_created_strength "
                  "  JOIN rif_effective_memories rif "
                  "    ON rif.memory_id = m.memory_id "
                  "  WHERE m.kind = 'LONG_TERM' "
                  "    AND rif.strength >= ?2 "
                  "    AND (m.created_at > 0 OR m.start_ts > 0) "
                  "  ORDER BY m.created_at DESC, m.start_ts DESC "
                  "  LIMIT ?3"
                  ") "
