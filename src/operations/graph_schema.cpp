#include "cortext/operations/graph_schema.hpp"

#include "cortext/buffered_write_instruction.hpp"
#include "cortext/processor/operation_context.hpp"

namespace cortext::operations
{

namespace
{
void
AddDdl (OperationContext &context, const char *sql)
{
  BufferedWriteInstruction op;
  op.query = sql;
  context.AddWriteInstruction (std::move (op));
}
} // namespace

void
EnsureGraphSchema::Execute (OperationContext &context) const
{
  // Graph storage (explicit tables; used by Algorithms 30–33).
  AddDdl (context,
          "CREATE TABLE IF NOT EXISTS graph_nodes ("
          "  node_id TEXT PRIMARY KEY,"
          "  type TEXT NOT NULL,"
          "  label TEXT,"
          "  embedding_id INTEGER,"
          "  metadata TEXT,"
          "  created_at INTEGER"
          ");");

  AddDdl (context,
          "CREATE TABLE IF NOT EXISTS graph_edges ("
          "  source_id TEXT NOT NULL,"
          "  target_id TEXT NOT NULL,"
          "  edge_type TEXT NOT NULL,"
          "  weight REAL NOT NULL DEFAULT 1.0,"
          "  decay_rate REAL,"
          "  last_reinforced INTEGER,"
          "  PRIMARY KEY (source_id, target_id, edge_type)"
          ");");

  AddDdl (context, "CREATE INDEX IF NOT EXISTS idx_graph_edges_source "
                   "ON graph_edges(source_id);");
  AddDdl (context, "CREATE INDEX IF NOT EXISTS idx_graph_edges_target "
                   "ON graph_edges(target_id);");
  AddDdl (context, "CREATE INDEX IF NOT EXISTS idx_graph_edges_type "
                   "ON graph_edges(edge_type);");

  // Name→node mapping for entities to avoid duplicates.
  AddDdl (context,
          "CREATE TABLE IF NOT EXISTS entity_index ("
          "  name TEXT PRIMARY KEY,"
          "  node_id TEXT NOT NULL"
          ");");

  // Optional goal nodes: either use type='goal' in graph_nodes or register here.
  AddDdl (context,
          "CREATE TABLE IF NOT EXISTS goal_nodes ("
          "  node_id TEXT PRIMARY KEY"
          ");");

  // Consolidation inputs/outputs (used by Alg 29c and Alg 30).
  AddDdl (context,
          "CREATE TABLE IF NOT EXISTS consolidation_summaries ("
          "  summary_id TEXT PRIMARY KEY,"
          "  summary_text TEXT,"
          "  cluster_size INTEGER"
          ");");
  AddDdl (context,
          "CREATE TABLE IF NOT EXISTS consolidation_sources ("
          "  summary_id TEXT,"
          "  source_text TEXT,"
          "  source_embedding_id INTEGER"
          ");");

  // Extraction outputs in relational form (populated by external workers).
  AddDdl (context,
          "CREATE TABLE IF NOT EXISTS extraction_entities ("
          "  summary_id TEXT NOT NULL,"
          "  name TEXT NOT NULL,"
          "  type TEXT NOT NULL,"
          "  salience REAL,"
          "  embedding_id INTEGER,"
          "  PRIMARY KEY (summary_id, name, type)"
          ");");

  AddDdl (context,
          "CREATE TABLE IF NOT EXISTS extraction_relations ("
          "  summary_id TEXT NOT NULL,"
          "  subject TEXT NOT NULL,"
          "  predicate TEXT NOT NULL,"
          "  object TEXT NOT NULL,"
          "  confidence REAL"
          ");");
}

} // namespace cortext::operations

