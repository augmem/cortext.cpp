  ...
  if (!fetched_any)
    {
      ...
      if (eligible.empty ()
        ...
        if (cfg.procedural_enabled && !disable_procedural_proactive)
          {
            const int min_proc = core::RetrievalProceduralSeedReserveCount (
                cfg.focus, cfg.sensitivity, cfg.stability, k);
            if (min_proc > 0)
              {
                std::vector<std::pair<double, Scored>> ranked_proc;
                ranked_proc.reserve (eligible.size ());
                for (const auto &candidate : eligible)
                  {
                    ...
                    ranked_proc.emplace_back (base_score (candidate),
                                              candidate);
                  }
                std::sort (ranked_proc.begin (), ranked_proc.end (),
                           [] (const auto &a, const auto &b) {
                ...
                const int take = std::min (
                    min_proc, static_cast<int> (ranked_proc.size ()));
                initial.reserve (static_cast<size_t> (take));
                for (int i = 0; i < take; ++i)
                  {
                    initial.push_back (ranked_proc[static_cast<size_t> (i)].second);
                  }
              }
          }
