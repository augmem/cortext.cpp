...
struct StreamingJSONParser::Impl
{
  ...
  // Structure tracking
  std::vector<std::pair<std::string, nlohmann::json>> stack;
  ...
  {
    if (active_branch_index)
      return required_fields;

    if (!oneof_branches.empty ())
      {
  ...
  BeginValueContext (const nlohmann::json &prop_schema)
  {
    ...
    // If key is discriminator, derive enum from branch values
    if (!current_enum_values && oneof_discriminator
  ...
  StoreValue (const nlohmann::json &value)
  {
    ...
    if (oneof_discriminator && !active_branch_index
    ...
    if (!active_branch_index && !oneof_branches.empty ())
      {
  ...
  CloseObject ()
  {
    if (!stack.empty () && stack.back ().first == "object")
      {
        stack.pop_back ();
        if (stack.empty ())
  ...
  ProcessChar (char c)
  {
    ...
    switch (state)
        ...
        if (c == '{')
          {
            ...
            stack.emplace_back ("object", nlohmann::json::object ());
      ...
      case JSONState::kExpectingValue:
        ...
        if (c == '{')
          {
            state = JSONState::kInNestedObject;
            stack.emplace_back ("object", nlohmann::json::object ());
            return true;
          }
        if (c == '[')
          {
            ...
            stack.emplace_back ("array", nlohmann::json::array ());
            return true;
          }
        return false;
      ...
      case JSONState::kInArray:
        ...
        if (c == ']')
          {
            if (!stack.empty () && stack.back ().first == "array")
              {
                stack.pop_back ();
          ...
          if (vt && *vt == "integer")
            {
              if (partial_value.empty ())
                allowed.push_back ("-");
              allowed.push_back (",");
              if (current_key.empty () || missing.empty ())
                allowed.push_back ("}");
            }
          else
            {
