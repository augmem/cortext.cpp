...
using namespace GemmaAudioConstants;
...
GemmaAudioFeatures
ExtractGemmaAudioFeatures (const float *pcm, size_t num_samples)
{
  using namespace GemmaAudioConstants;

  GemmaAudioFeatures features;
...
std::vector<GemmaAudioFeatures>
ChunkAndExtractFeatures (const float *pcm, size_t num_samples)
{
  using namespace GemmaAudioConstants;

  std::vector<GemmaAudioFeatures> chunks;
  ...
  while (offset < num_samples)
    {
      ...
      chunks.push_back (ExtractGemmaAudioFeatures (pcm + offset, chunk_samples));
