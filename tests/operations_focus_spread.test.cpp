           ...
           "[operations][focus_spread][feff]")
{
  ...
  OperationContext ctx_low (s, p_low, cfg, buf);
  ctx_low.SetCoherence (1.0); // isolate spread effect
  spread_op.Execute (ctx_low);
  feff.Execute (ctx_low);
